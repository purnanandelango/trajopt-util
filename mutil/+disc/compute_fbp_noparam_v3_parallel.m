function [Ak,Bk,wk,defect_traj,xbarprop] = compute_fbp_noparam_v3_parallel(tbar,xbar,ubar,t_burn,func,func_linz,ode_solver)
...
%   xbarprop      : nx x N
    ...
    % SETUP
    tspan_burn = cell(1,N-1);
    tspan_drift = cell(1,N-1);
    for k = 1:N-1
        tspan_burn{k} = [tbar(k), tbar(k) + t_burn];
        tspan_drift{k} = [tbar(k) + t_burn, tbar(k+1)];
    end    
