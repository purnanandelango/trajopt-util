	...
	# constraints
		...
		# obstacle avoidance
			...
			# linearization
				...
				# const θ1_full = [norm(ȳ_unobs[t+1][1:2] .- (p1 ./ scl_x1)) - r1/scl_x1 + (-1 * norm(ȳ_unobs[t+1][1:2])^2 + (p1 ./ scl_x1)⋅ȳ_unobs[t+1][1:2]) / norm(ȳ_unobs[t+1][1:2] .- (p1 ./ scl_x1)) for t=1:(NN-1)]
				const θ2_full = [norm(ȳ_unobs[t+1][1:2] .- (p2 ./ scl_x1)) - r2/scl_x1 + (-1 * norm(ȳ_unobs[t+1][1:2])^2 + (p2 ./ scl_x1)⋅ȳ_unobs[t+1][1:2]) / norm(ȳ_unobs[t+1][1:2] .- (p2 ./ scl_x1)) for t=1:(NN-1)]
				const θ3_full = [norm(ȳ_unobs[t+1][1:2] .- (p3 ./ scl_x1)) - r3/scl_x1 + (-1 * norm(ȳ_unobs[t+1][1:2])^2 + (p3 ./ scl_x1)⋅ȳ_unobs[t+1][1:2]) / norm(ȳ_unobs[t+1][1:2] .- (p3 ./ scl_x1)) for t=1:(NN-1)]			

				# const ϕ1_full = [SVector{nx1}(-1 .* (ȳ_unobs[t+1][1:2] .- (p1 ./ scl_x1)) ./ norm(ȳ_unobs[t+1][1:2] .- (p1 ./ scl_x1)) ) for t=1:NN-1]
				const ϕ2_full = [SVector{nx1}(-1 .* (ȳ_unobs[t+1][1:2] .- (p2 ./ scl_x1)) ./ norm(ȳ_unobs[t+1][1:2] .- (p2 ./ scl_x1)) ) for t=1:NN-1]
				const ϕ3_full = [SVector{nx1}(-1 .* (ȳ_unobs[t+1][1:2] .- (p3 ./ scl_x1)) ./ norm(ȳ_unobs[t+1][1:2] .- (p3 ./ scl_x1)) ) for t=1:NN-1]
