...
void mexFunction( int nlhs, mxArray *plhs[],
    ...
    for (i=0; i<mxGetNumberOfFields(MPARAMS); ++i) {
        ...
        if ( mxGetM(tmp1) != 80 || mxGetN(tmp1) != 1 ) {
            mexErrMsgTxt("MPARAMS.be must be matrix of dimension 80x1.");
        }
        copyMArrayToC(mxGetPr(tmp1),params.be,80 );
        ...
        if ( 0 == strcmp(fname1,"algoOuter")) {
            ...
            for (i2=0; i2<mxGetNumberOfFields(tmp1); ++i2) {
                ...
                if ( 0 == strcmp(fname2,"init")) {
                    ...
                    if ( mxGetM(tmp2) != 80 || mxGetN(tmp2) != 1 ) {
                        mexErrMsgTxt("MSETGS.algoOuter.init must be matrix of dimension 80x1.");
                    }
                    copyMArrayToC(mxGetPr(tmp2),settings.algoOuter.init,80);
                }
                else
                if ( 0 == strcmp(fname2,"maxit")) {
                    if (!mxIsDouble(tmp2)) {
    ...
    MRES = mxCreateStructMatrix(1,1, 5, fnames);
        tmp1 = mxCreateDoubleMatrix(80,1,mxREAL);
        copyCArrayToM( result.la, mxGetPr(tmp1), 80 );
