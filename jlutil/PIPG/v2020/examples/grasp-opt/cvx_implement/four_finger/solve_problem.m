...
for j=1:N-1
    ...
    Fcone(3,j) = norm(u([(j-1)*nu+7,(j-1)*nu+9])) - mu3*u((j-1)*nu+8);                          % friction cone            
    Fcone(4,j) = norm(u([(j-1)*nu+10,(j-1)*nu+12])) + mu4*u((j-1)*nu+11);
    ...
    Fmag(4,j) = norm(u((j-1)*nu+10:(j-1)*nu+12)) - F4max;                                       % grasp force magnitude
...
plot_soln(x,u,yy,N,s1,s2,s3,s4,a)