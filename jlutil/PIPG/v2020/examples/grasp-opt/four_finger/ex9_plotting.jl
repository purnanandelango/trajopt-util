...
function trajectory3D(x,u)
	...
	u4x = (u_scl .* [u[t][10] for t =1:N-1]) .+ rx[1:N-1]
	u4y = (u_scl .* [u[t][11] for t =1:N-1]) .+ ry[1:N-1]
	u4z = (u_scl .* [u[t][12] for t =1:N-1]) .+ rz[1:N-1]
	...
	for t=1:2:N-1
		...
		plot!([rx[t],u4x[t]],[ry[t],u4y[t]],[rz[t],u4z[t]],line=:solid,lw=1.1,color=:orange)
	end
