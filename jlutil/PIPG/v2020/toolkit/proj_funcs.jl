...
const kmax_admm = 1000

if isdefined(Main,:load_termination_criteria)
	# const ϵ_dual = copy(Main.ϵ_admm_dual)
	const ϵ_dual = 1e-5
else
	const ϵ_dual = 1e-5
end
...
# x, y and u may hold values from previous call to admm! (warm-starting)
	...
	κ2 .= κ2 .- z
	γ1 = norm(κ2,Inf)
	γ2 = max(γ1,norm(κ1,Inf))
	if γ2 ≤ ϵ
	    BLAS.blascopy!(n,z,1,x,1)
		BLAS.blascopy!(n,z,1,y,1)
		fill!(u,0.0)
		κ2[1] = 2.0 # Two projections so far
	else

		κ1 .= z .- u
		axpy!(α,y,κ1)
		BLAS.scal!(n,β,κ1,1)
		BLAS.blascopy!(n,x,1,κ2,1)				# For stop crit.
		f1(x,κ1)
		κ2 .= x .- κ2							# For stop crit.
		γ2 = norm(κ2,Inf)						# For stop crit.
		κ2 .= z .+ u
		axpy!(α,x,κ2)
		BLAS.scal!(n,β,κ2,1)
		f2(y,κ2)
		...
		κ2[1] = norm(κ1,Inf)					# For stop crit.

		γ1 = 1.0
		
		while κ2[1] ≥ ϵ || α*γ2 ≥ ϵ_dual
			κ1 .= z .- u
			axpy!(α,y,κ1)
			BLAS.scal!(n,β,κ1,1)
			BLAS.blascopy!(n,x,1,κ2,1)			# For stop crit.			
		...
		κ2[1] = 2*γ1 # Store the projection count
	end

end

function admm_v0!(x,y,u,z,f1,f2,α,β,ϵ,n,γ1,γ2,κ1,κ2)
# Projection onto intersection of two sets with respective projections defined by f1 and f2
# ---
# x  : projected result (copy 1)	(real n-vector) !
# y  : projected result (copy 2)	(real n-vector) !
...
# x, y and u may hold values from previous call to admm! (warm-starting)
	...
	if γ1 ≤ ϵ
		...
		f1(x,κ1)
		κ2 .= x .- κ2							# For stop crit.
		γ2 = infnorm(κ2,n,γ1)					# For stop crit.
		κ2 .= z .+ u
		axpy!(α,x,κ2)
		BLAS.scal!(n,β,κ2,1)
		f2(y,κ2)
		...
		# κ2[1] = max(κ2[1],α*γ2)				# for stop crit.
		κ2[1] = infnorm(κ1,n,γ1)				# for stop crit.
		...
		# while α*γ2 ≥ ϵ_dual  
			...
			f1(x,κ1)
			κ2 .= x .- κ2						# For stop crit.
			γ2 = infnorm(κ2,n,κ1[1])			# For stop crit.
			κ2 .= z .+ u
			axpy!(α,x,κ2)
			BLAS.scal!(n,β,κ2,1)
			f2(y,κ2)
...
function admm_v2!(x,z,f1,f2,α,β,ϵ,n,γ1,γ2,κ1,κ2,y,u)
# Projection onto intersection of two sets with respective projections defined by f1 and f2
# ---
# x  : projected result (copy 1)	(real n-vector) !
# z  : vector to be projected		(real n-vector)
...
# κ1 : temp storage variable 		(real n-vector) !
# κ2 : temp storage variable 		(real n-vector) !
# y  : projected result (copy 2)	(real n-vector) ! (set to z)
# u  : dual variable 				(real n-vector) ! (set to zeros)
...
function admm_old!(x,y,u,z,f1,f2,α,β,ϵ,n,γ1,γ2,κ1,κ2)
# Projection onto intersection of two sets with respective projections defined by f1 and f2
# ---
# x  : projected result (copy 1)	(real n-vector) !
# y  : projected result (copy 2)	(real n-vector) !
...
# κ  : temp storage variable        (real n-vector)              !
	...
	κ[1] = ψ*N # Store projection count
